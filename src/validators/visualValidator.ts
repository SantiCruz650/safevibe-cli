...
export async function serveDir(dir: string): Promise<{ origin: string; close: () => void }> {
  const server = http.createServer((req, res) => {
    const rel = decodeURIComponent((req.url || '/').split('?')[0]);
    const filePath = path.join(dir, rel === '/' ? 'index.html' : rel);
    if (!filePath.startsWith(dir)) { res.statusCode = 403; return res.end(); }
    ...
    try {
      ...
      while (Date.now() - started < SCENE_TIMEOUT_MS) {
        const ready = await page.evaluate(() => {
          ...
          if (!win.scene || !win.scene.children) return false;
          let count = 0;
          const stack: any[] = [win.scene];
          while (stack.length) {
            const o = stack.pop();
            if (!o) continue;
            if (o.isMesh) count++;
      ...
      try {
        ...
        if (meshInfo.error) {
          meshCount = -1;
          runtimeErrors.push(
            `Conteo de meshes fallo: ${meshInfo.error} | scene=${meshInfo.hasScene} ` +
          ...
          runtimeErrors.push(
            `Escena sin meshes | scene=${meshInfo.hasScene} children=${meshInfo.childrenCount} ` +
      ...
      try {
        const webglStatus = await page.evaluate(() => {
          ...
          try {
            if (win.renderer && win.renderer.domElement) {
              canvas = win.renderer.domElement as HTMLCanvasElement;
            }
          } catch (e) {}
          ...
          const samples: number[][] = [Array.from(pixel)];
          const positions = [
        ...
        if (webglStatus && !webglStatus.error) {
          ...
          webglInfo = `renderer=${webglStatus.renderer}, center=${JSON.stringify(
            webglStatus.centerPixel
          )}, allBlack=${webglStatus.allBlack}`;
        } else {
          webglInfo = (webglStatus && (webglStatus as any).error) || 'unknown';
      ...
      if (!isBlackScreen) {
        ...
        if (rendersNothing) {
          reasons.push(
            `WebGL no renderiza píxeles (contexto existe pero canvas vacío). ${webglInfo}`
          );
        } else {
