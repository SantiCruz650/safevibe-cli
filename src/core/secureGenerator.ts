...
export class SecureGenerator {
  ...
  public async generate(
    ...
    let currentCode = '';
    let isSecure = false;
    let extension = langChoice === 'py' ? '.py' : '.ts';
    let langKey = langChoice === 'py' ? 'python' : 'typescript';
    ...
    try {
      ...
      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        ...
        if (result.success) {
          ...
          const cleanError = rawError.replace(/^.*?sandbox\.(ts|py)\(\d+,\d+\):\s*/gm, '') || 'Error estructural.';
