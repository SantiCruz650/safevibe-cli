...
export class SecureGenerator {
    ...
    validateSimulationCode(code) {
        ...
        if (hudDeclarations > 0) {
            errors.push('ANTI-BUG: La variable "hud" YA ESTA declarada en el bootstrap del runtime. ' +
        ...
        if (/Float32BufferAttribute\s*\(/.test(code)) {
            ...
            if (bad2d) {
                errors.push('ANTI-BUG: Float32BufferAttribute requiere exactamente 3 componentes por punto (x, y, z). ' +
        ...
        if (!/animate\s*\(\s*\)/.test(code)) {
            errors.push('ANTI-BUG: El codigo no contiene la llamada animate() para iniciar el bucle de renderizado. ' +
            ...
            if (isSimulation) {
                ...
                if (simErrors.length > 0) {
                    ...
                    if (attempt < MAX_RETRIES) {
                        const repairPrompt = `Tu codigo JavaScript tiene los siguientes errores criticos que causaran pantalla negra:\n` +
